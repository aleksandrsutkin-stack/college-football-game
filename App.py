...
# ==============================================================================
# COLLEGE FOOTBALL MOGUL V23 — RESTORATION EDITION
# 1) RESTORED: Full Dashboard (Strategy, Staff, Facilities, Weekly Play).
# 2) FIX: Syntax Errors (Smart Quotes removed).
# 3) FIX: CFP Auto-Advance & Re-seeding logic.
# 4) FIX: Budget Guardrails & Idempotent History.
# ==============================================================================
...
# 1. CONSTANTS & CONFIG
try:
    st.set_page_config(page_title="CFB Mogul V23", page_icon="🏈", layout="wide")
except Exception:
    pass
...
def show_dashboard():
    ...
    # V23 RESTORATION: TABS & FULL DASHBOARD FEATURES
    ...
    with tab1:
        ...
        for p, v in st.session_state.roster.items():
            lab = f"{p}: {int(v)}"
            safe_progress(min(1.0, v / 100.0), lab)

    with tab2:
        st.markdown("### 🧢 Current Staff")
        # Simplified Staff UI for restoration
        for role, coach in st.session_state.staff.items():
             st.write(f"**{role}**: {coach.get('name','Coach')} (Off:{coach.get('off')} Def:{coach.get('def')})")
        if st.button("Hire New Staff (Auto-Fill)"):
             st.session_state.staff = {"HC": engine_generate_coach("HC",1), "OC": engine_generate_coach("OC",1), "DC": engine_generate_coach("DC",1)}
             st.success("Staff Hired")
             st.rerun()

    with tab3:
        st.write("### 🏟️ Facilities")
        c1, c2 = st.columns(2)
        c1.metric("Stadium Lvl", st.session_state.facilities["Stadium"])
        if c1.button("Upgrade Stadium ($10M)"):
    ...
    with tab4:
        ...
        if not st.session_state.schedule:
             ...
             for i, opp in enumerate(st.session_state.schedule):
                 res = next((x["Score"] for x in st.session_state.season_logs if x["Week"]==i+1), "Upcoming")
                 st.write(f"Week {i+1}: vs {opp} - {res}")
