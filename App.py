...
# ==============================================================================
# COLLEGE FOOTBALL MOGUL V20.1 — OFFSEASON FIX
# 1) Fixed NameError: 'show_offseason' is now defined and registered.
# 2) Safe Progress Bar: Wrapped st.progress for compatibility.
# 3) Budget Safety: HS Outreach cap auto-clamps to current budget.
# 4) All V20 Features (War Room, True Playoff) retained.
# ==============================================================================
...
# ==============================================================================
try:
    st.set_page_config(page_title="CFB Mogul V20.1", page_icon="🏈", layout="wide")
except Exception:
    pass
...
# ZONE 2: HELPERS
# ==============================================================================
def safe_progress(value: float, label: str = ""):
   """Streamlit compatibility wrapper (older versions lack st.progress(text=...))."""
   try:
...
def render_system_sidebar():
    with st.sidebar:
        ...
        st.caption(f"Version {STATE_VERSION} (Fixed)")
...
# ==============================================================================
# ZONE 6: UI / ROUTES (UN-INDENTED FIX)
# ==============================================================================
def show_offseason():
    sync_team_ratings()
    ...
    if st.session_state.offseason_step == 1:
        ...
        # V20: War Room (Dollar Input + Buttons)
...
# V20 FIX: The War Room (Budget Input + Plus/Minus)
def show_offseason_hs_outreach():
    ...
    for idx, pos in enumerate(POSITIONS):
        with cols[idx % 2]:
            ...
            # Badge logic
            badges = ""
            if pos in needs: badges += " <span class='badge badge-tier-f'>NEED</span>"
            if pos in hot: badges += " <span class='badge badge-tier-s'>PIPELINE</span>"
            
            # Card UI
            st.markdown(f"""
            <div class='game-card' style='padding: 8px; margin-bottom: 8px; display:flex; justify-content:space-between; align-items:center;'>
                <div style='font-weight:bold; font-size:1.1em;'>{pos} {badges}</div>
                <div style='font-family:monospace; font-size:1.2em;'>{helper_format_cash(current_amt)}</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            # Decrement
            if b1.button("➖", key=f"d_{pos}", disabled=(current_amt <= 0)):
                alloc[pos] = max(0, current_amt - step)
                st.rerun()
                
            # Bar chart visual
            share = (current_amt / new_cap) if new_cap > 0 else 0.0
            b2.progress(min(1.0, share))
            
            # Increment
            if b3.button("➕", key=f"i_{pos}", disabled=(remaining < step)):
                alloc[pos] = current_amt + step
                st.rerun()
    ...
    if st.button("Confirm & Run Recruiting 🚀", type="primary", disabled=(new_cap == 0)):
        if remaining > 0:
            st.error(f"You still have {helper_format_cash(remaining)} unassigned! Allocate it or lower your total budget.")
            return
