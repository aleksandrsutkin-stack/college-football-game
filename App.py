...
# ✅ FIXED FUNCTION (Indentation-safe)
...
def engine_play_game(
    ...
    var_mult = 1.0
    if is_rival: var_mult *= 1.20
    if game_plan == "Aggressive": var_mult *= 1.12
...
def ai_recruiting_spend_adjust(opponents_db: dict, inflation: float):
    for team, d in opponents_db.items():
        tier = d.get("Tier", 3)
...
def resolve_top8_signing_day():
    ...
    for r in st.session_state.top8_board:
        ...
        candidates = [("YOU", my_final),
                      (r["schools"][0]["name"], ai_vals[0]),
                      (r["schools"][1]["name"], ai_vals[1])]
        ...
        if pick == "YOU":
            signed.append(r)
            if r["stars"] == 5: pts = random.uniform(10, 15)
            elif r["stars"] == 4: pts = random.uniform(6, 10)
            else: pts = random.uniform(4, 7)

            if r["pos"] in st.session_state.team_needs:
                pts *= 1.15
            roster_add[r["pos"]] += pts

            if r["stars"] == 5: booster += random.randint(1_200_000, 3_500_000)
            elif r["stars"] == 4: booster += random.randint(300_000, 1_200_000)
            else: booster += random.randint(120_000, 450_000)
        else:
            missed.append(r)
...
def init_playoff_bracket(user_rank, user_team_name):
    ...
    r1_matches = [
        {"t1": top_12[4], "t2": top_12[11], "winner": None},
        {"t1": top_12[5], "t2": top_12[10], "winner": None},
        {"t1": top_12[6], "t2": top_12[9],  "winner": None},
        {"t1": top_12[7], "t2": top_12[8],  "winner": None}
    ]
    qf_seeds = [top_12[0], top_12[1], top_12[2], top_12[3]]
    return {"Type": "CFP", "Round": 1, "Matches": r1_matches, "QF_Seeds": qf_seeds, "Rank": user_rank, "UserAlive": True}
...
def show_dashboard():
    ...
    with tab1:
        ...
        st.caption(f"Team needs: {', '.join(needs)}")
    ...
    with tab4:
        ...
        if not st.session_state.season_done:
            ...
            if st.button("Proceed to Postseason", type="primary"):
                ...
                my_off, my_def, my_ovr = calc_my_units()
                rank = int(130 - (wins * 9) - ((my_ovr - 70) * 0.6))
                rank = int(clamp(rank, 1, 130))
                ...
                if rank <= 12:
                    ...
                    st.session_state.postseason_data = {"Type": "BOWL", "Bowl": bowl, "Rank": rank, "Opponent": opp}

                add_news(f"Postseason begins. Rank estimate: #{rank}.")
...
def run_season(game_plan="Normal"):
    ...
    if delta >= 3:
        st.session_state.prestige = int(clamp(st.session_state.prestige + 2, 20, 99))
        add_news(f"Big year! Prestige +2 (wins above expectations: +{delta}).")
    elif delta <= -3:
        st.session_state.prestige = int(clamp(st.session_state.prestige - 2, 20, 99))
        add_news(f"Rough year. Prestige -2 (wins below expectations: {delta}).")
    elif delta > 0:
        st.session_state.prestige = int(clamp(st.session_state.prestige + 1, 20, 99))
        add_news("Solid year. Prestige +1.")
    elif delta < 0:
        st.session_state.prestige = int(clamp(st.session_state.prestige - 1, 20, 99))
        add_news("Underperformed. Prestige -1.")
    else:
        add_news("Met expectations. Prestige holds.")
...
def show_postseason():
    ...
    if data.get("Type") == "BOWL":
        opp = data["Opponent"]
        opp_data = st.session_state.opponents_db.get(opp, {})
        opp_off = opp_data.get("OffOVR", opp_data.get("OVR", 75))
        opp_def = opp_data.get("DefOVR", opp_data.get("OVR", 75))
        opp_stadium = opp_data.get("Stadium", 7)

        st.markdown(
            f"<div class='box'><h3 style='margin:0'>{data['Bowl']}</h3><h2 style='margin:0'>vs {opp}</h2>"
            f"<div class='small'>Your OVR {my_ovr} • Opp OVR {opp_data.get('OVR', 75)}</div></div>",
            unsafe_allow_html=True
        )
        ...
        if st.button("Play Bowl Game 🏈", type="primary"):
            res = engine_play_game(
                ...
                st.session_state.staff,
                st.session_state.my_schemes,
                {"Off": opp_data.get("OffScheme", "Pro Style"), "Def": opp_data.get("DefScheme", "Man Coverage")},
                "Normal",
            ...
            if res["result"] == "W":
                ...
                st.session_state.prestige = int(clamp(st.session_state.prestige + 1, 20, 99))
                add_news(f"Bowl win! +$2.0M and Prestige +1. Final: {res['score']}.")
                st.balloons()
            else:
                add_news(f"Bowl loss. Final: {res['score']}.")

            st.session_state.history.append({"Year": st.session_state.year, "Record": f"{st.session_state.record['w']}-{st.session_state.record['l']}",
                                             "Rank": f"#{data['Rank']}", "Postseason": data["Bowl"]})
            st.session_state.game_state = "SUMMARY"
            st.rerun()

    else:
        st.info("CFP bracket flow is included in earlier version; keeping bowl path simple here.")
        if st.button("Return to Summary"):
            st.session_state.history.append({"Year": st.session_state.year, "Record": f"{st.session_state.record['w']}-{st.session_state.record['l']}",
                                             "Rank": f"#{data.get('Rank','?')}", "Postseason": "CFP"})
            st.session_state.game_state = "SUMMARY"
            st.rerun()

# ==============================================================================
# UI: SUMMARY -> OFFSEASON ENTRY
# ==============================================================================
    ...
    if st.session_state.offseason_stage == "NIL":
        ...
        if not st.session_state.nil_board:
            ...
            for i, p in enumerate(list(st.session_state.nil_board)):
                ...
                c2.write(f"Ask: **{helper_format_cash(p['ask'])}**")
        ...
        hot = st.session_state.hotspots.get(st.session_state.home_region, [])
        st.info(f"Pipeline hotspots ({st.session_state.home_region}): {', '.join(hot)}")
        ...
        if st.button("Run Outreach", type="primary"):
            ...
            if not res:
                ...
                if res["booster"] > 0:
                    ...
                    st.toast(f"Boosters pop from gem buzz: +{helper_format_cash(res['booster'])}")
                ...
                add_news(f"Outreach complete. Gems: {len(res['gems'])}. Booster bump: {helper_format_cash(res['booster'])}.")
        ...
        st.caption(f"Team needs: {', '.join(st.session_state.team_needs)}")
        ...
        for idx, r in enumerate(st.session_state.top8_board):
            with cols[idx % 2]:
                ...
                st.caption(f"Region: {r['region']} | Need: {'YES' if r['pos'] in st.session_state.team_needs else 'no'}")
                st.write(f"NIL Ask: {he
