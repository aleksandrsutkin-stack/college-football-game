...
Version 1.5.1 (Complete & Restored)

Changes:
- Restored all missing View functions (Dashboard, Postseason, etc.).
- Includes V1.5 Features: Retention Ransom, Hall of Fame, Expanded Universe.
- Includes V1.3/1.4 Fixes: Visual Bracket, Deterministic RNG.
"""
...
# ==============================================================================
# ZONE 1: CONFIGURATION & CONSTANTS
# ==============================================================================
...
class GameConfig:
    ...
    # V1.5: Expanded Universe (40+ Teams)
    REAL_WORLD_INIT = {
...
def render_cfp_bracket_tree(data: dict):
    ...
    if round_num == 1:
        ...
        for i, m in enumerate(display_order):
            ...
            winner = m.get("winner")
            matchup_class = "bracket-matchup" + (" completed" if winner else "") + (" user-involved" if user_team in [t1, t2] else "")
            html += f"<div class='{matchup_class}'>"
            html += f"<div class='bracket-team {'winner' if winner == t1 else ('loser' if winner else '')}'><span>{t1}</span>" + (f"<span class='bracket-score'>{s1}</span>" if s1 else "") + "</div>"
            html += f"<div class='bracket-team {'winner' if winner == t2 else ('loser' if winner else '')}'><span>{t2}</span>" + (f"<span class='bracket-score'>{s2}</span>" if s2 else "") + "</div></div>"
        html += "</div>"
        ...
        for i, seed_team in enumerate(qf_seeds):
            matchup_class = "bracket-matchup bracket-bye" + (" user-involved" if user_team == seed_team else "")
            html += f"<div class='{matchup_class}'><div class='bracket-team'><span><span class='bracket-seed'>{i+1}</span>{seed_team}</span></div><div style='text-align:center; color:#666; font-size:0.85em; margin-top:8px;'>BYE (awaits winner)</div></div>"
        html += "</div>"
        
        html += "<div class='bracket-round'><div class='bracket-round-title'>Semifinals</div>"
        html += "<div class='bracket-matchup' style='opacity:0.4;'><div style='text-align:center; padding:20px; color:#999;'>TBD</div></div>" * 2
        html += "</div></div>"
        ...
        for m in matches:
            ...
            s1, s2 = m.get("s1", ""), m.get("s2", "")
            winner = m.get("winner")
            matchup_class = "bracket-matchup" + (" completed" if winner else " active") + (" user-involved" if user_team in [t1, t2] else "")
            html += f"<div class='{matchup_class}'><div class='bracket-team {'winner' if winner == t1 else ('loser' if winner else '')}'><span><span class='bracket-seed'>{seed_map.get(t1, '?')}</span>{t1}</span>" + (f"<span class='bracket-score'>{s1}</span>" if s1 else "") + "</div>"
            html += f"<div class='bracket-team {'winner' if winner == t2 else ('loser' if winner else '')}'><span><span class='bracket-seed'>{seed_map.get(t2, '?')}</span>{t2}</span>" + (f"<span class='bracket-score'>{s2}</span>" if s2 else "") + "</div></div>"
        html += "</div>"
        
        html += "<div class='bracket-round'><div class='bracket-round-title'>Semifinals</div>"
        html += "<div class='bracket-matchup' style='opacity:0.6;'><div style='text-align:center; padding:20px; color:#999;'>Awaiting QF Results</div></div>" * 2
        html += "</div>"
        
        html += "<div class='bracket-round'><div class='bracket-round-title'>Championship</div>"
        html += "<div class='bracket-matchup' style='opacity:0.3;'><div style='text-align:center; padding:30px; color:#999;'>🏆</div></div>"
        html += "</div></div>"
        ...
        html += "<div class='bracket-matchup active' style='padding:30px;'><div style='text-align:center; font-size:2em;'>🏆</div><div style='text-align:center; color:#666; margin-top:10px;'>Awaiting Semifinal Results</div></div>"
        html += "</div></div>"
        ...
        for m in matches:
            ...
            winner = m.get("winner")
            matchup_class = "bracket-matchup" + (" completed" if winner else " active") + (" user-involved" if user_team in [t1, t2] else "")
            html += f"<div class='{matchup_class}' style='min-width:300px;'><div style='text-align:center; font-size:1.5em; margin-bottom:10px;'>🏆</div>"
            html += f"<div class='bracket-team {'winner' if winner == t1 else ('loser' if winner else '')}'><span><span class='bracket-seed'>{seed_map.get(t1, '?')}</span>{t1}</span>" + (f"<span class='bracket-score'>{s1}</span>" if s1 else "") + "</div>"
            html += f"<div class='bracket-team {'winner' if winner == t2 else ('loser' if winner else '')}'><span><span class='bracket-seed'>{seed_map.get(t2, '?')}</span>{t2}</span>" + (f"<span class='bracket-score'>{s2}</span>" if s2 else "") + "</div></div>"
        html += "</div></div>"
