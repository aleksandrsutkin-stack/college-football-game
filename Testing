...
VERSION 4.6 (The Ultimate UI Update)
...
STATE_VERSION = 4.6
NY6_BOWLS = ["Rose Bowl", "Sugar Bowl", "Orange Bowl", "Cotton Bowl", "Peach Bowl", "Fiesta Bowl"]
...
.staff-card { background: #ffffff; border: 1px solid #e0e0e0; border-radius: 10px; padding: 12px; text-align: center; box-shadow: 0 2px 6px rgba(0,0,0,0.06); }
.staff-role { font-size: 0.85em; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #546e7a; margin-bottom: 4px; }
.staff-name { font-size: 1.05em; font-weight: 800; color: #1a1a1a; }
...
/* MISC UI */
.top8-card-title { font-size: 1.15em; font-weight: 800; color: #1a1a1a; margin-bottom: 4px; }
.top8-card-trait { font-size: 0.85em; font-weight: 700; color: #6a1b9a; margin-left: 6px; }
...
@keyframes pulse-fade { 0%, 100% { opacity: 0.3; } 50% { opacity: 0.5; } }
...
def render_enhanced_staff_card(coach: dict, role: str, rating: int) -> str:
    ...
    trait_icon_html = f"<div class='staff-trait-icon'>{trait_icon}</div>" if trait_icon else ""
    trait_label_html = (
        "<div style='margin-top: 8px; font-size: 0.85em; color: #666;'>"
        f"<strong>Trait:</strong> {trait}</div>"
        if trait != "None"
        else ""
    )
...
def render_interest_meter(recruit: dict, chance: float, spend_by_pos: dict) -> tuple[str, str]:
...
def get_prestige_tier(prestige: int) -> tuple:
    if prestige >= 95: return ("🌟 BLUE BLOOD", "#FFD700")
    elif prestige >= 85: return ("⚔️ POWERHOUSE", "#C0C0C0")
    elif prestige >= 75: return ("📈 RISING PROGRAM", "#CD7F32")
    elif prestige >= 65: return ("🔨 REBUILDING", "#4CAF50")
    else: return ("🆘 STRUGGLING", "#f44336")
...
def render_trophy_shelf(shelf_name: str, trophies: List[Dict], max_display: int = 8, show_empty: bool = True) -> str:
    ...
    if show_empty:
        ...
        for _ in range(empty_count):
            trophy_items.append(
                f"<div class='trophy-item trophy-empty' style='animation: pulse-fade 2s infinite;' title='{category_data['empty_text']}'>"
                "<div class='trophy-icon' style='opacity: 0.15;'>❓</div>"
                "<div class='trophy-label' style='font-style: italic;'>Earn This</div>"
                "</div>"
            )
...
def process_postseason_security_boost():
    """
    Rewards the player for postseason success.
    Prevents the 'Stuck at 75%' job security ceiling by escalating elite outcomes.
    """
    last_result = st.session_state.get("last_postseason_result", "NONE")

    # 1. Championship Protection (The 'Saban' Rule)
    if last_result == "TITLE":
        st.session_state.job_security = 100.0
        add_news("🏆 CHAMPIONSHIP PROTECTED: You are untouchable in the eyes of the Boosters.")
        return

    # 2. CFP Appearance Boost
    if "CFP" in last_result:
        st.session_state.job_security = min(
            98.0,
            st.session_state.job_security + 15.0
        )
        add_news("⚔️ CFP BONUS: Playoff appearance has stabilized your job security.")

    # 3. Bowl Game Stabilization
    elif "BOWL" in last_result:
        if "WIN" in last_result:
            st.session_state.job_security = min(
                90.0,
                st.session_state.job_security + 8.0
            )
        else:
            # Bowl eligibility alone should prevent firing
            st.session_state.job_security = max(
                45.0,
                st.session_state.job_security + 3.0
            )

def update_prestige_after_season():
    """
    V4.6 NEW: Updates the 1-99 Prestige rating based on performance vs expectations.
    Ensures program growth is tied to results, not just tenure.
    """
    year = st.session_state.get("year", 0)
    # Prevent double-triggering prestige in the same year
    if st.session_state.get("prestige_last_updated_year") == year:
        return

    wins = safe_int(st.session_state.record.get("w", 0), 0)
    expected = safe_int(st.session_state.get("expected_wins", 6), 6)
    delta = wins - expected
    last_result = st.session_state.get("last_postseason_result", "NONE")
    change = 0

    # Win-Delta Logic
    if delta >= 3: change += 2      # Crushed expectations
    elif delta >= 1: change += 1    # Improved
    elif delta <= -4: change -= 3   # Disaster season
    elif delta <= -2: change -= 1   # Underperformed

    # Postseason Bonuses
    if last_result == "TITLE": change += 4
    elif "CFP" in last_result: change += 2
    elif last_result == "BOWL_WIN": change += 1
    elif last_result == "NO_BOWL": change -= 1

    if change != 0:
        st.session_state.prestige = max(1, min(99, st.session_state.prestige + change))
        add_news(f"Program Prestige adjusted to {st.session_state.prestige} ({'+' if change > 0 else ''}{change}).")

    st.session_state.prestige_last_updated_year = year
...
.bracket-bowl{font-size:0.75em;color:#666;text-align:center;margin-top:6px}
    ...
    if round_num == 1:
        ...
        for m in ordered_matches:
            ...
            bowl_display = m.get("bowl", "")
            bowl_html = f"<div class='bracket-bowl'>🏟️ {bowl_display}</div>" if bowl_display and bowl_display != "On-Campus" else ""
            html += f"<div class='{cls}'><div class='bracket-team {'winner' if m.get('winner')==m.get('t1') else 'loser' if m.get('winner') else ''}'><span>{m.get('t1')}</span><span class='bracket-score'>{m.get('s1','')}</span></div><div class='bracket-team {'winner' if m.get('winner')==m.get('t2') else 'loser' if m.get('winner') else ''}'><span>{m.get('t2')}</span><span class='bracket-score'>{m.get('s2','')}</span></div>{bowl_html}</div>"
        ...
        for m in matches:
            ...
            bowl_display = m.get("bowl", "")
            bowl_html = f"<div class='bracket-bowl'>🏟️ {bowl_display}</div>" if bowl_display and bowl_display != "On-Campus" else ""
            html += f"<div class='{cls}'><div class='bracket-team {'winner' if m.get('winner')==m.get('t1') else 'loser' if m.get('winner') else ''}'><span><span class='bracket-seed'>{seed_map.get(m.get('t1'),'?')}</span>{m.get('t1')}</span><span class='bracket-score'>{m.get('s1','')}</span></div><div class='bracket-team {'winner' if m.get('winner')==m.get('t2') else 'loser' if m.get('winner') else ''}'><span><span class='bracket-seed'>{seed_map.get(m.get('t2'),'?')}</span>{m.get('t2')}</span><span class='bracket-score'>{m.get('s2','')}</span></div>{bowl_html}</div>"
...
def init_playoff_bracket(user_rank, user_team_name):
    ...
    bowls = random.sample(NY6_BOWLS, 6)
    qf_bowls = bowls[:4]
    sf_bowls = bowls[4:]
    r1_matches = [
        {"seed_high": 5, "seed_low": 12, "t1": top12[4], "t2": top12[11], "winner": None, "bowl": "On-Campus"},
        {"seed_high": 6, "seed_low": 11, "t1": top12[5], "t2": top12[10], "winner": None, "bowl": "On-Campus"},
        {"seed_high": 7, "seed_low": 10, "t1": top12[6], "t2": top12[9], "winner": None, "bowl": "On-Campus"},
        {"seed_high": 8, "seed_low": 9, "t1": top12[7], "t2": top12[8], "winner": None, "bowl": "On-Campus"},
    ]
    return {
        "Type": "CFP",
        "Round": 1,
...
def organize_trophies_by_category(all_trophies: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    for trophy in all_trophies:
        ...
        elif "10+" in name or "10 Win" in name: categories["10+ Win Seasons"].append(trophy)
...
def migrate_state():
    ...
    defaults = {
        "year": 2026, "prestige": 60, "prestige_last_updated_year": None, "job_security": 75, "expected_wins": 6, "tenure": 1,
...
def show_offseason_top8_v8():
    ...
    for r in st.session_state.top8:
        ...
        with st.container(border=True):
            # Header
            trait = r.get("trait", "")
            trait_html = f"<span class='top8-card-trait'>{trait}</span>" if trait else ""
            st.markdown(
                f"<div class='top8-card-title'>{pos} {r['name']} ({r['rating']}){trait_html}</div>",
                unsafe_allow_html=True,
            )
            ...
            with c1:
                ...
                meter_html, rivalry_html = render_interest_meter(r, chance, {})
                st.markdown(meter_html, unsafe_allow_html=True)
                if rivalry_html:
                    st.markdown(rivalry_html, unsafe_allow_html=True)
                ...
                if st.button("Make Pitch", key=f"pitch_{rid}", disabled=disabled, use_container_width=True, type="primary"):
                    if not validate_budget_input(r["offer"], BudgetManager.get_current(), f"recruit {r['name']}"):
                        ...
                        if random.random() < chance:
                            st.balloons()
                            st.success(f"🎉 {r['name']} COMMITS TO {st.session_state.team_name}!")
                            time.sleep(1.5)
...
def show_dashboard():
    ...
    st.markdown(
        ...
        f"<h2>{st.session_state.team_name}</h2>"
        f"<div style='display: inline-block; background: {tier_color}; color: white; padding: 5px 15px; border-radius: 20px; font-weight: bold; margin-left: 10px;'>"
        f"{tier_name}</div></div>",
        unsafe_allow_html=True,
    )
    st.divider()
            ...
            for role in vacancies:
                ...
                for j, cand in enumerate(st.session_state.candidates[role]):
                    with cols[j]:
                        ...
                        st.markdown(
                            f"<div class='staff-card'><div class='staff-role'>{role}</div><div class='staff-name'>{cand['name']}</div><div class='small-muted'>OVR: {vis}</div><div style='font-weight:bold'>{helper_format_cash(cand['salary'])}</div></div>",
                            unsafe_allow_html=True,
                        )
                        b1, b2 = st.columns(2)
            ...
            with c3:
                ...
                if st.button("Sim Season", use_container_width=True, key="sim_season_main"):
                    with st.spinner("🏈 Simulating season..."):
                        remaining_games = max(0, 12 - st.session_state.week_index)
                        ...
                        for i in range(remaining_games):
                            ...
                            progress_bar.progress((i + 1) / max(1, remaining_games))
...
def show_season_end():
    ...
    st.markdown(f"<div class='resume-box'><div class='resume-grid'><div class='resume-item'><div class='resume-label'>Record</div><div class='resume-value'>{st.session_state.record['w']}-{st.session_state.record['l']}</div></div><div class='resume-item'><div class='resume-label'>SOS Score</div><div class='resume-value'>{avg_sos}</div></div><div class='resume-item'><div class='resume-label'>Best Win</div><div class='resume-value'>{best_win}</div></div><div class='resume-item'><div class='resume-label'>Worst Loss</div><div class='resume-value'>{worst_loss}</div></div></div></div>", unsafe_allow_html=True)
        ...
        if not user_match and round_num == 1:
            ...
            if st.button("Simulate Opening Round & Advance", type="primary"):
                ...
                if len(seeds) >= 4:
                    ...
                    qf_seeds = data.get("QF_Seeds", seeds[:4])
                    if len(winners) >= 4:
                         qf_bowls = data.get("QF_Bowls", [])
                         new_matches = [
                            {"t1": qf_seeds[0], "t2": winners[3], "winner": None, "bowl": qf_bowls[0] if len(qf_bowls) > 0 else ""}, # 1 vs 8/9
                            {"t1": qf_seeds[3], "t2": winners[0], "winner": None, "bowl": qf_bowls[1] if len(qf_bowls) > 1 else ""}, # 4 vs 5/12
                            {"t1": qf_seeds[2], "t2": winners[1], "winner": None, "bowl": qf_bowls[2] if len(qf_bowls) > 2 else ""}, # 3 vs 6/11
                            {"t1": qf_seeds[1], "t2": winners[2], "winner": None, "bowl": qf_bowls[3] if len(qf_bowls) > 3 else ""}, # 2 vs 7/10
                         ]
            ...
            if st.button("Advance to Next Round →", type="primary"):
                ...
                new_matches = []
                if len(winners) >= 2:
                    sf_bowls = data.get("SF_Bowls", [])
                    for i in range(0, len(winners), 2):
                        if i+1 < len(winners):
                            bowl_name = sf_bowls[i // 2] if len(sf_bowls) > i // 2 else ""
                            new_matches.append({"t1": winners[i], "t2": winners[i+1], "winner": None, "bowl": bowl_name})
...
def show_season_recap():
    ...
    if st.session_state.season_logs:
        best_game = max(
            st.session_state.season_logs,
            key=lambda g: g.get("OppOVR", 0) if "W" in g.get("Score", "") else 0
        )
        if "W" in best_game.get("Score", ""):
            st.success(f"🔥 **Signature Win:** Beat #{best_game['Opponent']} ({best_game['OppOVR']}) {best_game['Score']}")
        
        close_wins = []
        for g in st.session_state.season_logs:
            score = g.get("Score", "")
            if "W" not in score:
                continue
            try:
                final_score = score.split()[1]
                left, right = final_score.split("-")
                if abs(int(left) - int(right)) <= 7:
                    close_wins.append(g)
            except (IndexError, ValueError):
                continue
        if close_wins:
            st.warning(f"😅 **Nail-biters:** {len(close_wins)} wins decided by 7 or less")
    else:
        st.info("No games logged yet.")

    st.divider()
    if st.button("Proceed to Offseason ->", type="primary"):
        update_prestige_after_season()
